...
resx=camera_get_view_x(view_camera[0])+camera_get_view_width(view_camera[0])/2 - (width)/2
...
draw_text_outline(x+(width)/2,y+10,"Você está sem itens")// + string(pos_inv_name[pos_inv]))
draw_text(x+(width)/2,y+10,"Você está sem itens") //+ string(pos_inv_name[pos_inv]))
} else {
draw_set_valign(fa_top)
draw_set_halign(fa_left)
		...
		if pos_inv=0{
			...
			draw_text_outline(x+op_border+(op_border*3),y+op_border,_thp)
		...
		draw_text_outline(x+op_border+(op_border*3),y+op_border*2,_s)
		draw_text_color(x+op_border+(op_border*3),y+op_border*2,_s,c_green,c_green,c_green,c_green,1)
		}
		///desenhar a desc
		if member_item_mode=0{
		draw_text_outline(x+op_border,y+op_border+20,inv[pos_inv][pos_item].desc,10,110)
	...
	var _txx=x+(width)-(sprite_width)*5
	var _tyy= y+op_border + op_space*_yo
	draw_text_outline(_txx,_tyy,_name)
	draw_text_color(_txx,_tyy,_name,_c,_c,_c,_c,1)
	_yo++
	if _yo>_num-1{_yo=0}
	}
	//draw_text_outline(x+(sprite_width*width)-(sprite_width)*6,y+op_border + op_space*i,inv[pos_inv][i].name)
	//draw_text_color(x+(sprite_width*width)-(sprite_width)*6,y+op_border + op_space*i,inv[pos_inv][i].name,_c,_c,_c,_c,1)
	var _pos_it=pos_item+1 if array_length(inv[pos_inv])<=0{_pos_it=0}
	var _sname="("+string(_pos_it)+"/"+string(inv_max[pos_inv])+")"
	draw_text_outline(x+(width)-(sprite_width)*5,y+op_border + op_space*10,_sname)
	draw_text(x+(width)-(sprite_width)*5,y+op_border + op_space*10,_sname)
}
draw_set_valign(fa_top)
draw_set_halign(fa_left)
...
if options_item_mode=1 and member_item_mode=0{
	...
	draw_text_outline(x+op_border,y-op_border+(height)-(op_space*array_length(options_item))+op_space*i,options_item[i])
	draw_text_color(x+op_border,y+(height)-op_border-(op_space*array_length(options_item))+op_space*i,options_item[i],_c,_c,_c,_c,1)
	}
}
