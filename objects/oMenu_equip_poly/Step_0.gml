...
if (a_key or (accept_key and wfora)){_can_wfora= 1}
}
...
if cancel_key{if !ending and !wfora{ending=1} else if !ending and wfora{wfora=0 color_start = c_white color_end = c_yellow}}
...
if vender_mode=0{
	...
	var _old_a=oitem_manager.invp[member.actions,pos_inv]
	...
	//Atk Atk.E Def Res TenP Spd
	status[0]="" + string(member.atk) + status_pader(member.atk,_old_a.atk,_new_a.atk)
	status[1]="" + string(status_show.atke) + status_pader(member.atke,_old_a.atke,_new_a.atke)
	status[2]="" + string(status_show.def) + status_pader(member.def,_old_a.def,_new_a.def)
	status[3]="" + string(status_show.res) + status_pader(member.res,_old_a.res,_new_a.res)
	status[4]="" + string(status_show.ten_level) + status_pader(member.ten_level,_old_a.ten_level,_new_a.ten_level)
	status[5]="" + string(status_show.spd) + status_pader(member.spd,_old_a.spd,_new_a.spd)
	...
	if accept_key and !wfora{
		...
		oitem_manager.invp[member.actions,pos_inv]=_new_a
		...
		member.atke=(member.atke-_old_a.atke)+_new_a.atke
		member.def=(member.def-_old_a.def)+_new_a.def
		member.res=(member.res-_old_a.res)+_new_a.res
		member.ten_level=(member.ten_level-_old_a.ten_level)+_new_a.ten_level
		member.spd=(member.spd-_old_a.spd)+_new_a.spd
		audio_play_sound(equip,90,false,0.1)
		}
	}
} else {
	if array_length(inv[pos_inv])>0{
	status[0]="Atk:" + string(inv[pos_inv][pos].atk)
	status[1]="Atk.E:" + string(inv[pos_inv][pos].atke)
	status[2]="Def:" + string(inv[pos_inv][pos].def)
	status[3]="Res:" + string(inv[pos_inv][pos].res)
	status[4]="TenP:" + string(inv[pos_inv][pos].ten_level)
	status[5]="Spd:" + string(inv[pos_inv][pos].spd) 
	st_length= array_length(status)
